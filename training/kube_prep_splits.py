#!/usr/bin/env python3
"""Gate-check the kube pool and build kube_train_v6/kube_val_v6.
...
Inputs:
  ...
  incoming/kube_chains_*_train.jsonl   (3 read-shaped-chain form, v5)
  incoming/kube_nl_*_train.jsonl       (2 multi-family NL top-up, v5)
  incoming/kube_v6_*_train.jsonl       (3 four-form coverage batches, v6)
...
Output: prepared/kube_train_v6.jsonl (90%), prepared/kube_val_v6.jsonl
...
# 2 NL top-up + 3 v6 four-form batches, 2 eval-only
EXPECTED_INCOMING = (14, 2)
...
rng.shuffle(train)
rng.shuffle(val)
for name, items in [("kube_train_v6", train), ("kube_val_v6", val)]:
...
print("OK: gates passed, v6 splits written")
